	...
	for episode := 0; episode < episodes; episode++ {
		...
		for !isDone && currentStepInEpisode < maxStepsPerEpisode {
			...
			if board.CurrentPlayer == dqnAgentX.PlayerSymbol {
				...
				isDone = board.IsGameOver()
				// If the opponent ends the game, the reward for agent X
				// will be processed when agent X makes its next training step,
				// or through the last experience added to the buffer.
		...
		if board.CheckWin() {
			if board.CurrentPlayer == PlayerX { // The winner is the one who made the last move
				winsX++
			} else {
				winsO++
			}
