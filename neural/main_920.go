...
func NewDQNAgent(inputSize, outputSize, bufferCapacity int, playerSymbol int) *DQNAgent {
	qNet := NewNeuralNetwork(inputSize, []int{64, 64, 64}, outputSize, "sigmoid") // Пример архитектуры
	...
	return &DQNAgent{
		...
		Gamma:         0.95,  // Коэффициент дисконтирования 0.99
		Epsilon:       1.0,   // Начать с исследования
		MinEpsilon:    0.01,  // Минимальное значение эпсилон
		EpsilonDecay:  0.99999, // Скорость затухания эпсилон за шаг (очень медленная)
		LearningRate:  0.0003,
...
func main() {
	...
	// Параметры обучения
	episodes := 300000          // Количество игровых эпизодов для обучения
	maxStepsPerEpisode := 100 // Максимальное количество шагов за эпизод
	batchSize := 24           // Размер батча для обучения DQN 32
	...
	for episode := 0; episode < episodes; episode++ {
		...
		if (episode+1)%1000 == 0 {
			fmt.Printf("Эпизод: %d, Побед X: %d, Поражений X (Побед O): %d, Ничьих: %d, Эпсилон X: %.4f\n",
