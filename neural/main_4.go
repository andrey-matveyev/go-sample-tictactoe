...
func main() {
	...
	for episode := 0; episode < episodes; episode++ {
		...
		for !isDone && currentStepInEpisode < maxStepsPerEpisode {
			...
			if board.CurrentPlayer == dqnAgentX.PlayerSymbol {
				...
				// Reward is for agentX based on current outcome
				...
				if len(emptyCells) > 0 {
					...
					opponentAction = -1 // No available moves, implies draw
				}
	...
	// Example game after training - Corrected logic for winner identification and print order
	...
	for { // Loop indefinitely until game is over or max steps
		...
		// Determine whose turn it is for the current iteration
		var currentPlayerSymbol int
		if board.CurrentPlayer == dqnAgentX.PlayerSymbol {
			currentPlayerSymbol = dqnAgentX.PlayerSymbol
		} else {
		...
		if currentPlayerSymbol == dqnAgentX.PlayerSymbol {
			...
			action = dqnAgentX.ChooseAction(board)
			if action == -1 {
				// If no moves are available, it's a draw.
				// GetGameOutcome will correctly identify it as a draw.
				isOver, winner = board.GetGameOutcome()
				break
			}
		...
		// This should always be true if action is not -1
		if !moveSuccessful {
